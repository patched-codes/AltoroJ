...
**
	...
	public static String transferFunds(String username, long creditActId, long debitActId, double amount) {
			...
			try {
				...
				String debitTransactionType = (debitAccount.getAccountId() == userCC) ? "Cash Advance" : "Withdrawal";
				String creditTransactionType = (creditAccount.getAccountId() == userCC) ? "Payment" : "Deposit";
				...
				preparedStatement.setString(3, debitTransactionType);
				...
				preparedStatement.setString(7, creditTransactionType);
				...
				if (debitAccount.getAccountId() == userCC){
					String cashAdvanceFeeSQL = "INSERT INTO TRANSACTIONS (ACCOUNTID, DATE, TYPE, AMOUNT) VALUES (?, ?, 'Cash Advance Fee', ?)";
					preparedStatement = connection.prepareStatement(cashAdvanceFeeSQL);
					preparedStatement.setLong(1, debitAccount.getAccountId());
					preparedStatement.setTimestamp(2, date);
					preparedStatement.setDouble(3, CASH_ADVANCE_FEE);
					preparedStatement.execute();
					debitAmount += CASH_ADVANCE_FEE;
					Log4AltoroJ.getInstance().logTransaction(String.valueOf(userCC), "N/A", CASH_ADVANCE_FEE);
				}
							
				String updateDebitAccountSQL = "UPDATE ACCOUNTS SET BALANCE = ? WHERE ACCOUNT_ID = ?";
				preparedStatement = connection.prepareStatement(updateDebitAccountSQL);
				preparedStatement.setDouble(1, debitAccount.getBalance() + debitAmount);
				preparedStatement.setLong(2, debitAccount.getAccountId());
				preparedStatement.execute();
				
				String updateCreditAccountSQL = "UPDATE ACCOUNTS SET BALANCE = ? WHERE ACCOUNT_ID = ?";
				preparedStatement = connection.prepareStatement(updateCreditAccountSQL);
